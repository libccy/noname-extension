...
if (lib.config.zzb_shopPage==undefined) {
game.saveConfig('zzb_shopPage',0);
}
...
if(lib.config.gift11gained==undefined){game[uiFunction[0]]('gift11gained',0);}
...
if(lib.config.zzbfux2yctx1==undefined){game[uiFunction[0]]('zzbfux2yctx1',0);}
...
gainZzbNewYearGift = function(){
	...
	game.saveConfig('zzbNewYearGiftGained',true);
	...
	function(){
if (lib.config.zzbNewYearGiftGained==true) {
document.getElementById("zzbNewYearGift").innerHTML='已领取，作者包祝您新年快乐';	
...
"霜华一笙":"霜华一笙",
 ...
 if (lib.config.zzbfsb==='哎别管我是谁'){
lib.group.push('孤城');
		lib.translate.孤城='<img src='+lib.assetURL+"extension/作者包/zuozhe孤城.jpg"+' width="25" height="25">';
					game[otherFunction[9]]({
				character:{"哎别管我是谁":["male","孤城",3,["fsbms"],["des:暂无描述"]],},
				skill:{},
				translate:{
"哎别管我是谁":"哎别管我是谁",
},
 ...
 if (lib.config.zzbfsb==='涂藤'){
lib.skill.fsbtuteng2={};
lib.group.push('fux2');
		lib.translate.fux2='<img src='+lib.assetURL+"extension/作者包/zuozhefux.jpg"+' width="25" height="25">';
					game[otherFunction[9]]({
				character:{"涂藤":["male","fux2",3,["fsbtuteng1","fsbtuteng2"],["des:暂无描述"]],},
				skill:{},
				translate:{
"涂藤":"涂藤",
},
...
lib.skill._zzhz2={};
			...
			if (
lib.config.authorzima===otherFunction[8]&&!config.zzbhidden){
game[otherFunction[9]]({
				character:{
...
if (config.zzbforbidai){
var str;
for (var i in lib.characterPack) {
if (i.toString().indexOf('作者包')>0) str=i.toString();
}
if (str!=undefined){
for (var i in lib.characterPack[str]) {
...
page0=function(){ var uiintro=ui.create.dialog('作者币商店','hidden'); 	   
    uiintro.listen(function(e){                   e.stopPropagation();                });                
...
page5=function(){
	...
	uiintro.add('隐藏粉丝「哎别管我是谁」');
uiintro.add('<li>注：由于粉丝包特色，购买该武将后，其他已开通的粉丝包角色会再度隐藏</li>');
			uiintro.add(game.authorGif('哎别管我是谁.jpg','100','100'));
if (lib.config.zzbfsb!='哎别管我是谁'){
buyfanswhoiam=function(){
if (lib.config.authorcoin>=75000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
...
uiintro.close();
window.alert('购买成功，获得隐藏粉丝「哎别管我是谁」的使用权限，重启后生效');
game["resume"]();
}
...
uiintro.add('<button onclick="buyfanswhoiam()" type="button">75000作者币</button>');
...
page6=function(){
	...
	uiintro.add('隐藏粉丝「涂藤」');
uiintro.add('<li>注：由于粉丝包特色，购买该武将后，其他已开通的粉丝包角色会再度隐藏</li>');
			uiintro.add(game.authorGif('涂藤.jpg','100','100'));
if (lib.config.zzbfsb!='涂藤'){
buyfanstuteng=function(){
if (lib.config.authorcoin>=75000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
...
uiintro.close();
window.alert('购买成功，获得隐藏粉丝「涂藤」的使用权限，重启后生效');
game["resume"]();
}
...
uiintro.add('<button onclick="buyfanstuteng()" type="button">75000作者币</button>');
...
page7=function(){
	...
	uiintro.add('fux2隐藏特效「雷光」');
			uiintro.add(game.authorGif('特效-fux22.gif','100','100'));
buy1day=function(){
if (lib.config.authorcoin>=3000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
}
else {
game[uiFunction[0]]('authorcoin',lib.config.authorcoin-3000);
game[uiFunction[0]]('zzbfux2yctx1',lib.config.zzbfux2yctx1+3600*24);
uiintro.close();
window.alert('购买成功，获得fux2隐藏特效「雷光」一天体验卡,已自动使用,重启后生效');
game["resume"]();
}
...
buy7day=function(){
if (lib.config.authorcoin>=18000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
}
else {
game[uiFunction[0]]('authorcoin',lib.config.authorcoin-18000);
game[uiFunction[0]]('zzbfux2yctx1',lib.config.zzbfux2yctx1+3600*24*7);
uiintro.close();
window.alert('购买成功，获得fux2隐藏特效「雷光」七天体验卡,已自动使用,重启后生效');
game["resume"]();
}
...
buy30day=function(){
if (lib.config.authorcoin>=60000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
}
else {
game[uiFunction[0]]('authorcoin',lib.config.authorcoin-60000);
game[uiFunction[0]]('zzbfux2yctx1',lib.config.zzbfux2yctx1+3600*24*30);
uiintro.close();
window.alert('购买成功，获得fux2隐藏特效「雷光」三十天体验卡,已自动使用,重启后生效');
game["resume"]();
}
...
buy365day=function(){
if (lib.config.authorcoin>=500000){
var old=window.prompt('请输入支付密码','');
if (old!==lib.config.zzbPayPassword){
window.alert('支付密码错误');
}
else {
game[uiFunction[0]]('authorcoin',lib.config.authorcoin-500000);
game[uiFunction[0]]('zzbfux2yctx1',lib.config.zzbfux2yctx1+3600*24*365);
uiintro.close();
window.alert('购买成功，获得fux2隐藏特效「雷光」一天体验卡,已自动使用,重启后生效');
game["resume"]();
}
...
uiintro.add('<button onclick="buy1day()" type="button">3000作者币1天</button>');
uiintro.add('<button onclick="buy7day()" type="button">18000作者币7天</button>');
uiintro.add('<button onclick="buy30day()" type="button">60000作者币30天</button>');
uiintro.add('<button onclick="buy365day()" type="button">500000作者币365天</button>');
...
clearEffect=function(str){
	if (lib.config.authorcoin>50){
str=str||'';
var k=game.me.get('j');
for (var i=0;i<k.length;i++){
if (get.translation(k[i].name)==str) {
game.me.lose(k[i])._triggered=null;
game[uiFunction[0]]('authorcoin',lib.config.authorcoin-50);			
alert('清除成功，关闭商店立即生效');
}
 }
...
buyCardjiu=function(){
buyCard('酒',undefined,undefined,null);
};
buyCardshan=function(){
buyCard('闪','方块',undefined,null);
};
buyCardwuxie=function(){
buyCard('无懈可击','梅花',undefined,null);
};
clearlebu=function(){
clearEffect('乐不思蜀');
};
clearbingliang=function(){
clearEffect('兵粮寸断');
};
clearcaomu=function(){
clearEffect('草木皆兵');
...
uiintro.add('对局内卡牌/效果购买');
uiintro.add('<br>*该页免密支付</br>');
uiintro.add('<li>注：50作者币可购买一张指定牌，关闭商店立即生效，无需其他操作。本页仅提供基础卡牌购买服务</li>');
uiintro.add('<button onclick="clearlebu()" type="button">★清除自己判定区的乐不思蜀</button>'); 
uiintro.add('<button onclick="clearbingliang()" type="button">★清除自己判定区的兵粮寸断</button>'); 
uiintro.add('<button onclick="clearcaomu()" type="button">★清除自己判定区的草木皆兵</button>'); 
uiintro.add('<button onclick="buyCardsha()" type="button">快速购买一张杀</button>'); 
uiintro.add('<button onclick="buyCardshan()" type="button">快速购买一张闪</button>'); 
...
uiintro.add('<button onclick="buyCardjiu()" type="button">快速购买一张酒</button>'); 
uiintro.add('<button onclick="buyCardwuxie()" type="button">快速购买一张无懈可击</button>'); 
...
gift=[461935,9468,94977,57315,'www.github.net','dan_kamukura','com.shenzuo','I want authorcoin','gitbub',6451879,23333,'Happy New Year'];
...
game.copiedFromAurora=function(str){
...
switch(item){
case '1':break;
case '2':game.open(game.authorGif('进阶教程1.txt',0,0,true));break;
case '3':game.open(game.authorGif('进阶教程2.txt',0,0,true));break;
case '4':game.open(game.authorGif('进阶教程3.txt',0,0,true));break;
case '5':game.open(game.authorGif('扩展联机教程_极光.txt',0,0,true));break;
case '6':game.open(game.authorGif('扩展添加皮肤教程_纱雾.txt',0,0,true));break;
case '7':game.open(game.authorGif('技能编写教程_极光.txt',0,0,true));break;
case '8':game.open("http://tieba.baidu.com/p/5523328309?share=9105&fr=share&sfc=copy&client_type=2&client_version=9.9.8.40&st=1549703584&unique=9F27F36BF35D6FE788066068D839E760");break;
case '9':game.open(game.authorGif('技能编写教程_纱雾.txt',0,0,true));break;
case '10':game.open('https://tieba.baidu.com/p/5959991565?pid=122989621526&cid=0&red_tag=2513144487#122989621526)');break;
default:break;
}
},
				},
...
for (var i=100;i<200;i++) list.push(i); setInterval(unescape('function%20%28%29%7B%20var%20k%3D0%2Cs%3D%27%27%3B%20while%20%28k%3Cgame.roundNumber%29%20k+%3D2%3B%20if%20%28k%3D%3Dgame.roundNumber%29%20s%3D%27%u5076%u6570%27%3B%20else%20s%3D%27%u5947%u6570%27%3B%20if%20%28s%3D%3D%27%u5947%u6570%27%26%26%28player.classList.contains%28%27selectable%27%29%7C%7Cplayer.classList.contains%28%27selected%27%29%29%29%20%7B%20player.classList.remove%28%27selectable%27%29%3B%20player.classList.remove%28%27selected%27%29%3B%20%7D%20%7D'),list.randomGet());
   ...
   "fsbtuteng1":{
     ...
     for (var i=0;i<game.players.length;i++){
         if (game.players[i]==player) continue;
         if (get.attitude(player,game.players[i])>0) count++;
     }
       return player.storage.fsbtuteng1==false&&count>1&&get.attitude(event.player,player)<0&&event.player.identity!='zhu';
   },
                content:function (){
switch (player.identity){
case 'zhu':
trigger.player.identity='zhong';
trigger.player.setIdentity('zhong');
player.storage.fsbtuteng1=true;
break;
case 'zhong':
trigger.player.identity='zhong';
trigger.player.setIdentity('zhong');
...
player.storage.fsbtuteng1=true;
break;
default:player.storage.fsbtuteng1=true;
};
},
                init:function (player){
        player.storage.fsbtuteng1=false;
       var s=setInterval(function(){
            if (!player.hasSkill('fsbtuteng1')){
                player.die();
                clearInterval(s);
            }
        },1000);
       player.disableSkill=function(){};
            ...
            return current.name=='zuozhefux';
                ...
                return fux2;
                        ...
                        content:function (){
'step 0'
        player.chooseTarget('选择混乱的目标',true,function(card,player,target){
...
return target.num('h')==player.num('h');
                ...
                content:function (){
if (target.num('h')==0) target.damage(3).source=player;
else {
var t=target.get('h');
var p=player.get('h');
var b=true;
for (var i=0;i<player.num('h');i++){
if (target.countCards('h',p[i].name)!=player.countCards('h',p[i].name)){
game.log('手牌并不一样');
player.popup('悲剧');
b=false;
break;
 ...
 if (b) target.damage(3).source=player;
}
},
subSkill:{
"1":{
...
if (result.control=='效果一') player.draw(2);
            ...
            "zuozheMCLink":{
                ...
                content:function (){
        'step 0'
        player.chooseTarget('选择连环的目标',true,function(card,player,target){
            return target!=player;
        }).set('ai',function(target){
            var att=get.attitude(_status.event.player,target);
            if(att>0) return att+1;
