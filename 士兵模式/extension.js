game.import("extension",{name:"士兵模式",content:function (config,pack){
    if(['identity','guozhan','versus'].contains(lib.config.mode)){
        var skill={
		soldier:{
		...
		soldier3:{
			...
			content:function(){
				...
				if(result.control!='cancel2'&&result.control){
					...
					trigger.untrigger();
					trigger.finish();
                    if(!player.storage.soldier.length) player.unmarkSkill('soldier');
...
},help:{"士兵模式":"<ul><li>游戏开始阶段，场上所有角色随机获得3名士兵。<li>准备阶段，你获得一名士兵的技能。<li>当你即将受到伤害时，你可以选择一名士兵替你承受此次伤害，然后失去此士兵<li>轻步兵：结束阶段，你回复一点体力。<li>重步兵：锁定技，结束阶段，你回复一点体力；黑【杀】对你无效；你计算与其他角色的距离时始终+1。<li>轻骑兵：你使用的【杀】可额外指定一个目标。<li>重骑兵：你使用的【杀】可额外指定一个目标。锁定技，目标角色需要额外出一张【闪】才能闪避你的【杀】；你计算与其他角色的距离时始终+1。<li>弓箭兵：锁定技，你的攻击范围无限；你不能对距离1以内的角色使用【杀】。<li>强弩兵：锁定技，你的【杀】无视防具；你的攻击范围+1；距离2以外的角色不能成为你【杀】的目标。<li>长枪兵：你的【杀】可指定距离2以内的目标。<li>圆盾兵：锁定技，你不能成为点数小于8的【杀】的目标。<li>刀剑兵：锁定技，你使用的点数大于8的杀不可闪避<li>水军：你防止非属性伤害；你受到的属性伤害+1。<li>军医：出牌阶段限一次，你可以令一名其他角色恢复一点体力。<li>火箭兵：你可以将一张普通【杀】当作火【杀】使用。锁定技，你使用的火【杀】无距离限制，且需要额外出一张【闪】才能闪避；你使用火【杀】指定目标后，失去一点体力。<li>斥候：出牌阶段开始时，你可以观看一名角色的手牌<li>通信兵：出牌阶段，你可以给其他角色一张牌，并摸一张牌；其他角色可以在其出牌阶段给你一张牌，并摸一张牌。<li>运输兵：出牌阶段，你可以观看牌堆顶的X张牌，并其中获得Y张牌（X为你体力上限，Y为你当前体力值）。<li>轻车兵：锁定技，其他角色计算与你的距离时始终-1；你的【杀】指定目标后，视为对其他在攻击范围内的所有角色各使用了一张【杀】。<li>重车兵：锁定技，其他角色计算与你的距离时始终-1；你的【杀】指定目标后，视为对其他在攻击范围内的所有角色各使用了一张【杀】；距离1以内的角色不能指定你成为【杀】目标；你不能使用装备牌；你计算与其他角色的距离时始终+1。<li>斧兵：你使用【杀】指定目标后，你可以弃置其防具。你的【杀】指定目标时，若其没有防具，你对其造成伤害+1。<li>器械兵：你每失去一次装备区的牌，可立即将一名其他角色的装备牌置于你的装备区（不可替换已有装备）<li>骑射手：你使用的【杀】可额外指定一个目标。锁定技，你的攻击范围无限；你不能对距离1以内的角色使用【杀】；你的【杀】指定目标后须进行一次判定：若为♠，该【杀】无效。<li>蜀兵：判定阶段和结束阶段前，你可以令一名蜀势力角色摸一张牌<li>魏兵：判定阶段和结束阶段前，你可以令一名魏势力角色摸一张牌<li>吴兵：判定阶段和结束阶段前，你可以令一名吴势力角色摸一张牌<li>群兵：判定阶段和结束阶段前，你可以令一名群势力角色摸一张牌"},config:{},package:{
    character:{
        character:{
        },
        translate:{
