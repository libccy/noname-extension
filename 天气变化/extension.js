game.import("extension",{name:"天气变化",content:function (config,pack){
    if(_status.video||_status.connectMode) return;
    _status.weatherchance=parseFloat(config.chance)||0.5;
...
var life;
            if(_status.weather=='qing'||Math.random()<parseFloat(config.noqing)){
                var weathers=['yu','shuang','bao','wu','xue','feng','lei'];
                if(lib.config.mode!='chess'){
                    weathers.remove('xue');
                }
                _status.weather=weathers.randomGet();
                life=JSON.parse(config.duration||'[4,4]');
            }
            else{
                _status.weather='qing';
                life=JSON.parse(config.qingduration||'[2,4]');
            }
...
},help:{"天气变化":"<ul><li>晴<br>没有任何事情发生<li>雨<br>在一名角色的回合结束后，若场上没有洪水，有一定机率将一张洪水置于其判定区<li>霜<br>每当一名角色受到火焰伤害，有一定机率令此伤害-1<li>雾<br>所有角色使用的杀有一定机率失效<li>雹<br>每名角色在回合结束后有一定机率受到一点伤害<li>雪<br>（战棋模式）每名角色在回合结束后有一定机率随机移动1格<li>风<br>当一名角色受到火焰伤害后，有一定机率令距离其1以内的一名随机角色受到一点火焰伤害<li>雷<br>在一名角色的回合结束后，若场上没有闪电，有一定机率将一张闪电置于其判定区"},config:{"noqing":{"name":"异常天气出现概率","init":"0.5","item":{"0.1":"10%","0.3":"30%","0.5":"50%","0.7":"70%","0.9":"90%"}},"chance":{"name":"天气效果触发概率","init":"0.5","item":{"0.1":"10%","0.2":"20%","0.3":"30%","0.5":"50%","0.8":"80%"}},"duration":{"name":"异常天气持续时间","init":"[4,4]","item":{"[2,4]":"1~3回合","[4,4]":"3~6回合","[4,7]":"3~9回合","[7,4]":"6~9回合","[7,7]":"6~12回合"}},"qingduration":{"name":"晴朗天气持续时间","init":"[2,4]","item":{"[2,4]":"1~3回合","[4,4]":"3~6回合","[4,7]":"3~9回合","[7,4]":"6~9回合","[7,7]":"6~12回合"}}},package:{
    character:{
        character:{
        },
        translate:{
