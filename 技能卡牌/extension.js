game.import("extension",{name:"技能卡牌",content:function (config,pack){
    lib.arenaReady.push(function(){
        ...
        if(!_status.video){
            list=list.randomGets(Math.ceil(lib.card.list.length*(parseFloat(lib.config['extension_技能卡牌_num'])||0.05)));
            for(var i=0;i<list.length;i++){
                lib.card.list.push([suit.randomGet(),Math.ceil(Math.random()*13),list[i]+'_charactercard']);
            }
        }
    });
},precontent:function (){
    
},help:{"技能卡牌":"<ul><li>牌堆中随机加入5%的技能牌<li>出牌阶段对自己使用，随机获得卡牌对应角色的一项技能直到使用者的下一个出牌阶段开始<li>一个角色最多只能通过技能卡牌获得一个技能，新获得技能后将失去之前以此法获得的技能<li>不能获得主公技、限定技、觉醒技等特殊技能，以及场上只能唯一存在的技能<li>若卡牌对应角色没有可获得的技能，目标摸两张牌"},config:{"num":{"name":"卡牌比例","init":"0.05","item":{"0.02":"2%","0.05":"5%","0.1":"10%","0.2":"20%"}}},package:{
    character:{
        character:{
        },
        translate:{
