...
skill:{	
				...
				content:function(){
					...
					if(list.length&&event.parent.parent.parent.name!='phaseUse'){
						...
						player.chooseButton(dialog,function(button){
							...
							if(!player.num('h','sha')) return (button.link[2]=='sha')?1:-1;		
				...
				content:function(){
					...
					if(event.current!=player){
						event.current.chooseControl('质疑于吉','不质疑于吉',function(){
							...
							if(_status.event.target.num('h')>3) return '不质疑于吉';
							if(_status.event.target.num('h')<2) return '质疑于吉';
	 ...
	 "xwj_xsanguo_shoushu":{
       ...
       'step 0'
        ...
        'step 1'
         ...
         game.broadcastAll()+trigger.player.node.avatar.setBackgroundImage('extension/新武将/xwj_xsanguo_zhangjiao.jpg');        
    ...
    event.finish();
     ...
     "step 2"
        ...
        "step 3"
       player.chooseToDiscard('he','什么？要弃置一张牌？').ai=function(card){
...
game.import('character',function(){
            ...
            "xwj_xhuoying_shuimen":["male","xhuo",3,["xwj_xhuoying_luoxuan","xwj_xhuoying_shanjie","xwj_xhuoying_refengyin"],[]],
            ...
            "xwj_xhuoying_zuojin":["male","xhuo",3,["xwj_xwugeng_weishou"],[]],
...
skill:{
	"xwj_xwugeng_weishou2":{
        ...
        return player.hp>0;
                ...
                content:function (){
        'step 0'
        player.chooseControl('虎眈','墨霞','cancel2',function(){
            var player=_status.event.player;
            if(player.hp>1){
                return '虎眈';
            }
        ...
        'step 1'
        if(result.control=='虎眈'){                       
         event.goto(2);               
        }
        else if(result.control=='墨霞'&&player.countCards('e')>0){
            var chat=['忍法·超兽伪画','书上说，微笑能增加人与人之间的交流'].randomGet();
            player.say(chat); 
            player.discardPlayerCard(player,true,'e');   
            // player.choosePlayerCard(player,'e',true);
        ...
        'step 2'                 
          player.chooseTarget('选择【虎眈】的目标',lib.translate.xwjweishou_info,true,function(card,player,target){
            return target!=player;
        }).set('ai',function(target){
        ...
        'step 3'
        if(result.bool){
           var chat=['封印术·虎视眈弹','安息吧，你们这些亡魂'].randomGet();
            player.say(chat);  
            var target=result.targets[0];
            player.line(target,'green');           
			target.addTempSkill('xwj_xwugeng_weishou2',{player:'phaseUseBegin'}); 
...
audio:"ext:新武将:2",
  ...
  animationColor:"water",
     filter:function (event,player){
      return event.player.hp<=0&&event.player.identity!='zhu'&&event.player!=player;
       },       
         logTarget:"player", 			
         ...
         var myid=player.identity;
            ...
            myid='zhong'
                ...
                trigger.player.update();   		
          		...
          		player.maxHp=6;
         		 player.hp=6;		 
         		 game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/新武将/xwj_xhuoying_yuyi.jpg'); 
 ...
 "xwj_xhuoying_shuidun":{
    ...
    if(player.hp<1) return 10;
            ...
            "xwj_xhuoying_jinshu":{
                ...
                forced:true,
                popup:false,
               derivation:['xwj_xhuoying_fenshen','xwj_xhuoying_shanjie','xwj_xhuoying_zhuansheng'],               
                content:function (){
...
var skill=trigger.player.skills.randomGet()
    ...
    var chat=['吃我一拳，看是你的骨头硬还是我的拳头硬','点解？点解？点解要逼我出手？'].randomGet();
        ...
        return 6-get.equipValue(card);
                ...
                content:function (){   
		player.discardPlayerCard(player,true,'e').ai=get.buttonValue;
		
        //1 player.discardPlayerCard(player,'e',true).ai=get.buttonValue; 修复了旧卡死BUG  

		//2 if(target.countCards('he',{type:'equip'})){
...
translate:{
            ...
            "xxiao":"晓",   
			   ...
			   "xwj_xwugeng_weishou":"伪兽",
			   "xwj_xwugeng_weishou2":"虎",
			   "xwj_xwugeng_weishou2_info":"不能使用或打出牌",
			   "xwj_xwugeng_weishou_info":"结束阶段，你可选择一项：墨霞：你将一张装备区的牌收进手牌区，然后回复一点体力并摸一张牌；虎耽：你选择一名其他角色，令其直到下个出牌阶段开始，不能使用或打出牌<font color=#F0F>封印术·虎视眈弹</font>，然后你视为对其使用一张【杀】，结算后其随机使用一张装备牌<font color=#F0F>配合团藏</font>",
           ...
           "xwj_xhuoying_lunmu":"轮墓",
            ...
            "xwj_xhuoying_fuzhi_info":"游戏开始所有角色摸牌后、准备阶段开始前和结束阶段，你可以选择一名存活角色，获得其所有能获得技能，直到下一次更改为止",
            ...
            "xwj_xhuoying_shanjie_info":"<font color=#F0F>飞雷神</font> 锁定技，你的防御距离始终+1，你的进攻距离无限",
...
characterIntro:{
     	"xwj_xwugeng_tian":"天，龙族，本名“黑龙”，香港漫画《封神纪》及中国大陆改编3D动画《武庚纪》中登场的男性角色。神族之主，亦称“天帝”。在神族中地位特殊，而且寿命远远超过其余神众。 平时生活在龙的体内，拥有无色界神力。无色界神力是神力中最罕见的一个种类，用于压抑声音、气味、力气、光、攻击、防御等，让对手的全部能力大幅降低、黯然无色。是上古与冥族交战取得世界支配权的远古神族，被不死鸟称为是“黑龙之主”，与白龙、黄龙、赤龙、飞龙同为远古神族五大古龙之一。",
					"xwj_xwugeng_zhengshan":"真禅圣王，港漫《封神纪》五圣王之一，主长生界神力，绝招：生死轮回限、大千世界、空镜映月等，神技——因果转轮。",
					"xwj_xwugeng_shixing":"十刑，《封神纪》（动画《武庚纪》）中的人物，天之子，阎部大神，号称“猎神之神”，主修罗界神力，后得到不死鸟的帮助使用元始界神力。被大剑士子羽用诛天剑杀死后再次复活，但这次复活也恢复了神志和记忆，从而踏上向神族复仇之路",
					"xwj_xwugeng_xuanfeng":"玄风圣王，港漫《封神纪》五圣王之一，主万象界神力，能瞬间引发超强风暴，曾一招将十刑吹到千里之外，也用此招从古龙手里救走心月葵。绝招：暴风、银翼破空、无形剑，神技——真空。",
				},
	...
	derivation:['xinliegong'],
        ...
        for(var i=0;i<event.cards.length;i++){
            if(get.type(event.cards[i])=='trick'||get.type(event.cards[i])=='delay'){
                player.draw(2);
				player.addTempSkill('xwj_xwugeng_xuemao2',{player:'phaseEnd'});
           ...
           if(get.type(event.cards[i])=='equip'){
                player.recover();
				player.addTempSkill('xinliegong',{player:'phaseEnd'});
                event.cards[i].discard();
                event.cards.splice(i--,1);
                event.finish();
            }
	 ...
	 "xwj_xwugeng_baiqian":{
		...
		"step 1"
	   	game.delay(2);
      game.broadcastAll()+ui.background.setBackgroundImage("extension/新武将/wms_background1.jpg");
...
content:function (){
    ...
    ai:{
     ...
     setTimeout(function(){
    player.maxHp++;  
    ui.backgroundMusic.src=lib.assetURL+'extension/新武将/wms_backgroundmusic.mp3'; 
	game.broadcastAll()+ui.background.setBackgroundImage("extension/新武将/wms_shixing_background.jpg");
 ...
 translate:{
           ...
           "xwj_xwugeng_shensu_info":"锁定技，你的进攻距离＋X（X为你的体力值），你的防御距离+Y（Y为你损失的体力值）",           
            ...
            "xwj_xwugeng_zhengkong_info":"<font color=#F0F>神技•真空</font> 当你流失体力或一次受到大于一点的伤害后，你可选择令一名其他非主公角色，在其回合结束起计时，九九八十一秒后，其因窒息而死亡。<font color=#F0F>与十刑互相克制</font> ",
          	 ...
          	 "xwj_xwugeng_xuemao2":"血矛",
          	 "xwj_xwugeng_xuemao_info":"出牌阶段限一次，你可以展示牌堆顶的一张牌，若该牌为：装备牌，你回复一点体力，获得技能【烈弓】直到回合结束；锦囊牌，你摸两张牌，且使用的【杀】可指定任意名角色，直到回合结束；基本牌，你可视为对任意一名其他角色使用一张不计距离、不计入次数限制的【杀】。然后将这张展示的牌置入弃牌堆",	 
          	 "xwj_xwugeng_xuemao2_info":"你使用的【杀】可指定任意名角色，直到回合结束",
          	 "xwj_xwugeng_baiqian":"白墙",
            "xwj_xwugeng_baiqian_info":"<font color=#F0F>无色界神力</font><br><li> 锁定技，游戏开始阶段，你令所有其他角色的所有技能失效，直到其受到伤害为止<br><li>出牌阶段限一次，你可以选择一名其他角色，令其所有技能失效，直到其回合结束；且不能使用或打出牌，直到其回合开始<br><li>锁定技，当你的手牌数与体力值不相等时，你不能成为【杀】或锦囊牌的目标",
            ...
            "xwj_xwugeng_baiqian2_info":"出牌阶段限一次，你可以选择一名其他角色，令其所有技能失效，直到其回合结束；且不能使用或打出牌（直到其回合开始为止）",
            ...
            "xwj_xwugeng_zhuanlun_info":"锁定技，限定技，游戏开始所有角色摸牌或你的回合开始后，你令所有其他角色获得技能“因果”",
...
connect:true,		
                ...
                image:"ext:新武将/xwj_xus_houzi.png",
                enable:true,
                selectTarget:-1,
                filterTarget:function (card,player,target){
        return target==player;
...
image:"ext:新武将/zbfs.png",				
        ...
        target.addJudge(card,cards);
            ...
            return 0;
                ...
                image:"ext:新武将/xwj_xus_mianju.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,               
...
image:"ext:新武将/xwj_xus_shoulijian.png",				
                ai:{
                    basic:{
                        order:9,
                        value:6,
            ...
            "xwj_xus_kuwu":{
                audio:true,
                fullskin:true,
				image:"ext:新武将/xwj_xus_kuwu.png",
                type:"equip",
                subtype:"equip1",
                skills:["xwj_xus_kuwu"],
                nomod:true,
            ...
            "xwj_xus_xuelunyang":{
                ...
                image:"ext:新武将/xwj_xus_xuelunyang.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,                
            ...
            "xwj_xus_jiuwei":{
                ...
                image:"ext:新武将/xwj_xus_jiuwei.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,               
...
if(!lib.config.cards.contains('xwj_xus_equip')) lib.config.cards.push('xwj_xus_equip');
};
},help:{"新武将":"<li>此扩展始创于2017年8月，汇集了部分三国新武将、《火影忍者》、《秦时明月》、《封神纪》等作品的人物，技能强度略高，可联机。<li>新增卡牌：【手里剑】2张，【写轮眼】、【九尾】、【漩涡面具】、【苦无】、【猴子】各1张<li>游戏时请关闭“火影忍者”武将栏的新版替换开关，否则会有部分武将的技能会缺失<li>游戏时若遇见卡死情况，打开兼容模式提高扩展的兼容性即可解决。<li>【编码】Sukincen<li>【配图】Sukincen<li>【录制配音】Sukincen"},config:{},package:{
    character:{
        character:{
        },
        translate:{
