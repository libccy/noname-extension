...
game.import('character',function(){
            ...
            "xwj_xhuoying_liudaoxianren":["male","qun",Infinity,["xwj_xhuoying_renzong"],[]],
...
skill:{
	"xwj_xhuoying_renzong":{
       ...
       'step 0'
            player.logSkill('xwj_xhuoying_renzong',trigger.player);                       
                player.chooseControl('xwj_xhuoying_tianyan','xwj_xhuoying_xianti').set('prompt','令'+get.translation(target)+'获得一项技能');
                goon=true;                    
        ...
        'step 1'
		 ...
		 player.maxHp=6;
         player.hp=6;		 
         game.broadcastAll()+trigger.player.node.avatar.setBackgroundImage('extension/新武将/xwj_xhuoying_yuyi.jpg');        
    ...
    "xwj_xhuoying_renzong2":{
        ...
        player.line(target,'green');
		target.gain(2,get.cardPile(function(card){
...
translate:{
 ...
 "xwj_xhuoying_qianshou":"千手",
            ...
            "xwj_xhuoying_renzong_info":"游戏开始时，你的体力值为无限。当一名其他角色进入濒死状态时，你可以选择技能【天眼】或【仙体】，令其永久获得之，其回复体力至2并摸两张牌，然后你的体力上限和体力值改为6，失去技能【忍宗】。当该被授予忍宗的角色造成或受到一次伤害后，你随机获得两张基本牌",
			"xwj_xhuoying_tianyan":"天眼",
            "xwj_xhuoying_tianyan_info":"回合开始阶段，你可以选择一名角色，然后获得其一项技能，直到回合结束",			
...
skill:{
 "xwj_xwugeng_shensu":{
    ...
    content:function (){
        ...
        "step 3"
        if(result.bool){
           var chat=['打打杀杀，真没意思','不用感谢我'].randomGet();
            player.say(chat);    
...
"xwj_xu_xuelunyang":{
 audio:"ext:新武将:1",
    ...
    content:function (){
        ...
        if(event.isMine()){
            event.dialog=lib.skill.xwj_xu_xuelunyang.createDialog(player,target);//tianshu
            event.switchToAuto=function(){
                event._result=event.skillai(event.list);
                game.resume();
            };
...
},help:{"新武将":"<li>此扩展始创于2017年8月，汇集了部分三国新武将、《火影忍者》、《秦时明月》、《封神纪》等作品的人物，技能强度略高，可联机，但版本必须要一致，若不一致请先到游戏内更新到最新版本的扩展。若有更好的技能设计或建议可在贴吧留言哦！<li>新增卡牌：【手里剑】2张，【写轮眼】、【九尾】、【漩涡面具】、【苦无】、【猴子】各1张<li>游戏时请关闭“火影忍者”武将栏的新版替换开关，否则会有部分武将的技能会缺失<li>游戏时若遇见卡死情况，打开兼容模式提高扩展的兼容性即可解决。目前可能会引起卡死情况的武将有；“志村团藏”的“惑梦”不开兼容模式会卡死，此时只需打开兼容模式即可……<li>【编码】Sukincen<li>【配图】Sukincen<li>【录制配音】Sukincen"},config:{},package:{
    character:{
        character:{
        },
        translate:{
