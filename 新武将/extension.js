...
var chat=['我都说了，要打倒我，就要先找到蜃的实体','海市蜃楼，风景这边独好……'].randomGet();
 ...
 player.useCard({name:'sha'},trigger.target,false);       
        ...
        return target != player;
                ...
                content:function () {
        "step 0"
        player.$fullscreenpop('驱敌寇，保林叶', 'fire'); 
...
if(lib.device||lib.node){
	...
	game.import('character',function(){
      ...
      "xwj_xwugeng_shixing":["male","shen",3,["xwj_xwugeng_fuhuo","xwj_xwugeng_fuchou"],[]],
					...
					"xwj_xwugeng_tianwu":["male","shen",4,["xwj_xwugeng_pili"],[]],
	...
	"xwj_xwugeng_pili":{               
        ...
        if(event.targets.length){
            ...
            event.redo();
                ...
                var num=0;
                for(var i=0;i<game.players.length;i++){
                    if(game.players[i].isDamaged()&&ai.get.attitude(player,game.players[i])>2){
                        num++;
                    }
                }    
                if(num<2) return 0;
...
content:function (){
 ...
 translate:{
	 ...
	 "xwj_xwugeng_pili":"霹雳",
	 "xwj_xwugeng_pili_info":"出牌阶段限一次，你可弃置任意张相同颜色的手牌，并对X名攻击范围内的其他角色造成X点伤害（X为你以此法弃置的手牌数）", 
