...
skill:{
	"xwj_xwugeng_weishou2":{
        ...
        'step 2'                 
          player.chooseTarget('选择【虎眈】的目标',lib.translate.xwj_xhuoying_weishou_info,true,function(card,player,target){
            return target!=player;
        }).set('ai',function(target){
        ...
        if(result.bool){
           var chat=['封印术·虎视眈弹','安息吧，你们这些亡魂'].randomGet();
            player.say(chat);    
            result.targets[0].equip(get.cardPile(function(card){
            return get.type(card,'equip')=='equip';
        }),'gain2');
            player.logSkill('xwj_xhuoying_weishou');  			
			result.targets[0].addTempSkill('xwj_xhuoying_weishou2',{player:'phaseBegin'}); 
...
translate:{
            ...
            "xxiao":"晓",   
			   ...
			   "xwj_xwugeng_weishou_info":"结束阶段，你可选择一项：墨霞：你将一张装备区的牌收进手牌区，然后回复一点体力并摸一张牌；虎耽：你令一名其他角色随便使用一张装备牌，然后直到该角色的回合开始，其不能使用或打出牌，你再视为对其使用一张【杀】",
