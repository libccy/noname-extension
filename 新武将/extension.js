    ...
    var chat=['我要贯彻我的忍道：只要努力，即使不会忍术也能打败天才！','这招本来是为了打败宁次而准备的，今天破例让你见识见识'].randomGet();
        ...
        trigger.num+=game.roundNumber;
            ...
            "xwj_xhuoying_rexuanfeng":{
                ...
                forced:true,
                filter:function (event,player){
        return player.isMaxHandcard(true)&&event.card&&event.card.name=='sha'&&event.notLink();
    },
    filter:function (event){
        return ;
    },
...
player.chooseTarget('选择【碎岳】的目标',lib.translate.xwj_xwugeng_suiyue_info,true,function(card,player,target){
       return player.canUse({name:'sha'},target,false)&&target!=player&&target.hp>player.hp;
        }).set('ai',function(target){
            return -get.attitude(_status.event.player,target);            
        });        
        "step 2"
...
content:function (){
 ...
 translate:{
  ...
  "xwj_xwugeng_suiyue_info":"<font color=#F0F>神技•天斗碎岳</font>出牌阶段限一次，若你的体力值不为全场最高之一，你可以失去一点体力，然后选择一名体力值大于你的其他角色，视为对其连续使用【杀】，直到其体力值比你的小为止",
	 ...
	 "xwj_xwugeng_pili_info":"<font color=#F0F>神技•真武大霹雳</font> 每两轮限一次，若你的体力值不大于当前存活的角色数，你可弃置X张相同颜色的手牌，并对X名攻击范围内的其他角色各造成X点伤害（X为你以此法弃置的手牌数）", 
           ...
           "xwj_xwugeng_shensu_info":"锁定技，你的进攻距离＋X（X为你的体力值），你的防御距离+Y（Y为你损失的体力值）",           
            ...
            "xwj_xwugeng_baiqian_info":"<font color=#F0F>无色界神力</font><br><li> 锁定技，游戏开始阶段或你进入游戏时，你令所有其他角色的所有技能失效，直到其受到伤害为止<br><li>出牌阶段限一次，你可以选择攻击范围内的一名其他角色，令其所有技能失效（直到其回合结束）；且不能使用或打出牌（直到其回合开始）<br><li>锁定技：①你使用的【杀】可指定任意名角色; ②当你的手牌数与体力值不相等时，你不能成为【杀】或锦囊牌的目标",
...
},help:{"新武将":"<li>此扩展始创于2017年8月，汇集了部分三国新武将、《火影忍者》、《秦时明月》、《封神纪》等作品的人物，技能强度略高，可联机。<li>新增卡牌：【手里剑】2张，【写轮眼】、【九尾】、【漩涡面具】、【苦无】、【猴子】各1张<li>游戏时请关闭“火影忍者”武将栏的新版替换开关，否则会有部分武将的技能会缺失<li>开始游戏时或游戏过程中若遇见卡死情况，打开兼容模式提高扩展的兼容性即可解决。目前千手柱间的[木遁]在牌堆剩一张牌时发动会卡死，仍待修复<li>【编码】Sukincen<li>【配图】Sukincen<li>【录制配音】Sukincen"},config:{},package:{
    character:{
        character:{
        },
        translate:{
