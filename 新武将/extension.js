...
skill:{	
 ...
 "xwj_xsanguo_wuniang":{
        ...
        if(result.bool){
            ...
            target.draw();  
          //  player.storage.xwj_xsanguo_wuniang=target;            
           // var guan=game.findPlayer(function(current){
       //     return current.name=='guansuo';
      //  });
       //   if(guan){
        //     current.draw();
     //     }
   ...
   for(var i=0;i<game.players.length;i++){
        ...
        return player.hp==1&&event.source&&event.source!=player&&event.source.sex=='male';        
                ...
                content:function (){
        'step 0'                               
        trigger.source.chooseBool('你是否愿意变身为关索娶鲍三娘为妻？').set('ai',function(){                    
       ...
       'step 1'
        ...
        trigger.source.maxHp+5-trigger.source.maxHp;
        }
        else{
        trigger.source.maxHp+4-trigger.source.maxHp;
        }
        trigger.source.update();
        player.logSkill('xwj_xsanguo_xushen2');  
    ...
    ai:{
	 ...
	 "xwj_xwugeng_suiyue":{
        ...
        var chat=['祭司那边需要我的帮助，我必须速战速决','凡人，你很有本领，那就接我最强一招吧'].randomGet();
            player.say(chat); 
        "step 1"
player.chooseTarget('选择【碎岳】的目标',lib.translate.xwj_xwugeng_suiyue_info,true,function(card,player,target){
       return player.canUse({name:'sha'},target,false)&&target!=player&&!target.hasSkill('xiangle')&&!target.hasSkill('new_xiangle')&&target.hp>player.hp;
        }).set('ai',function(target){
            return -get.attitude(_status.event.player,target);            
        });        
        "step 2"
...
content:function (){
 ...
 translate:{
  ...
  "xwj_xwugeng_suiyue_info":"<font color=#F0F>神技•天斗碎岳</font>出牌阶段限一次，若你的体力值不为全场最高之一，你可以失去一点体力，然后选择一名体力值大于你的其他角色，视为对其连续使用无视防具的【杀】，直到其体力值比你的小为止",
	...
	/*game.import('character',function(){
			var xxiaoyuan={
				...
				connect:true,
				character:{
			"xwj_xwj_xxiaoyuan_mssyj":["female","qun",4,["xwj_xxiaoyuan_shengge","xwj_xxiaoyuan_xiemu"],[]],
            "xwj_xwj_xxiaoyuan_ylwg":["male","qun",4,["xwj_xxiaoyuan_jianzhi"],[]],
 ...
 "xwj_xxiaoyuan_shengge":{
      ...
      // player.addTempSkill('mzshengge2','shaAfter');    
        ...
        player.chooseTarget('选择【圣歌】的目标',lib.translate.xwj_xxiaoyuan_shengge_info,true,function(card,player,target){
            ...
            return -get.attitude(event.player,target);            
        });        
        "step 2"
        if(result.bool){
...
if(lib.device||lib.node){
				for(var i in xxiaoyuan.character){xxiaoyuan.character[i][4].push('ext:新武将/'+i+'.jpg');}
			}else{
				for(var i in xxiaoyuan.character){xxiaoyuan.character[i][4].push('db:extension-新武将:'+i+'.jpg');}
			}
			return xxiaoyuan;
		});
		lib.config.all.characters.push('xxiaoyuan');
		if(!lib.config.characters.contains('xxiaoyuan')) lib.config.characters.push('xxiaoyuan');
		lib.translate['xxiaoyuan_character_config']='魔法少女';
		*/
