﻿﻿﻿game.import("extension",function(lib,game,ui,get,ai,_status){return {name:"群雄乱舞",editable:false,content:function (config,pack){
	 // ---------------------------------------定义势力------------------------------------------//
	lib.group.push('shen');
	 ...
	 "xwj_xsanguo_shoushu":{
       ...
       'step 0'
        ...
        'step 1'
         ...
         game.broadcastAll()+trigger.player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xsanguo_zhangjiao.jpg');        
...
if(lib.device||lib.node){
				for(var i in xsanguo.character){xsanguo.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xsanguo.character){xsanguo.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xsanguo;
		});
         ...
         var myid=player.identity;
            ...
            myid='zhong'
                ...
                trigger.player.update();   		
          		...
          		player.maxHp=6;
         		 player.hp=6;		 
         		 game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xhuoying_yuyi.jpg'); 
 ...
 audio:"ext:群雄乱舞:1",
    ...
    check:function (event,player){
        ...
        //if(result.bool){           
            ...
            setTimeout(function(){
  game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xhuoying_xieshen.jpg'); 
            player.node.avatar.show('extension/群雄乱舞/xwj_xhuoying_xieshen.jpg');
},100);
        //}                             
    },   
            },
        ...
        game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xhuoying_feiduan.jpg');              
        player.update();
      }                  
    },
                forced:true,
 ...
 "xwj_xhuoying_guazhang":{
  ...
  //  },
        ...
        var chat=['灵化之术————归位！！！','终于回来了，刚才真危险啊！'].randomGet();
               player.say(chat);        
        var target=player.storage.xwj_xhuoying_linghua;              
        game.swapPlayer(player,target);  
        player.storage.xwj_xhuoying_linghua=false;
	...
	derivation:['xwj_xhuoying_lunmu'],
       ...
       var chat=['那是一个只有和平，只有胜利，只有爱的世界','我已将我的力量给了你，将来再还这个恩情','拥有森罗万象之力，开山裂海，实力堪比尾兽，就凭我宇智波斑这个完全体须佐能乎'].randomGet();
        ...
        if(result.bool){ 
			...
			game.delay(0.5);
			game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xhuoying_liudaoban.jpg');             
...
translate:{
           ...
           "xwj_xhuoying_lunmu":"轮墓",
            ...
            "xwj_xhuoying_fuzhi_info":"游戏开始所有角色摸牌后或你进入游戏时、你的准备阶段开始前和结束阶段，你可以选择一名存活角色，获得其所有能获得技能，直到下一次更改为止",
...
if(lib.device||lib.node){
				for(var i in xhuoying.character){xhuoying.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xhuoying.character){xhuoying.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xhuoying;
		});
...
if(lib.device||lib.node){
				for(var i in xqinshi.character){xqinshi.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xqinshi.character){xqinshi.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xqinshi;
		});
 ...
 "xwj_xwugeng_yinguo":{
  ...
  /*  init:function(player){
    ...
    */
	...
	game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xwugeng_eshan.jpg'); 		
...
content:function (){
	...
	game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xwugeng_zhengshan.jpg'); 
	'step 1'
        var evt=_status.event.getParent('phase');
        if(evt){
            game.resetSkills();
    ...
    player.maxHp++;  
    ui.backgroundMusic.src=lib.assetURL+'extension/群雄乱舞/wms_backgroundmusic.mp3'; 
   	game.broadcastAll()+ui.background.setBackgroundImage("extension/群雄乱舞/wms_shixing_background.jpg");
    game.broadcastAll()+player.node.avatar.setBackgroundImage('extension/群雄乱舞/xwj_xwugeng_xinshixing.jpg'); 	     
  ...
  "xwj_xwugeng_suiyue_info":"<font color=#F0F>神技•天斗碎岳</font>出牌阶段限一次，若你的体力值不为全场最高之一，你可选择一名体力值不小于你的其他角色，然后你失去一点体力，视为对其连续使用无视防具的【杀】，直到其体力值比你的小为止",
	 ...
	 "xwj_xwugeng_pili_info":"<font color=#F0F>神技•真武大霹雳</font> 每两轮限一次，若你的体力值不大于当前存活的角色数，你可弃置X张相同颜色的手牌，并对X名攻击范围内的其他角色各造成X点伤害（X为你以此法弃置的手牌数）", 
           ...
           "xwj_xwugeng_shensu_info":"<font color=#f00>锁定技</font> 你的进攻距离＋X（X为你的体力值），你的防御距离+Y（Y为你损失的体力值）",           
			...
			if(lib.device||lib.node){
				for(var i in xwugeng.character){xwugeng.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xwugeng.character){xwugeng.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xwugeng;
		});
...
if(lib.device||lib.node){
				for(var i in xxiaoyuan.character){xxiaoyuan.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xxiaoyuan.character){xxiaoyuan.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xxiaoyuan;
		});
...
if(lib.device||lib.node){
				for(var i in xu.character){xu.character[i][4].push('ext:群雄乱舞/'+i+'.jpg');}
			}else{
				for(var i in xu.character){xu.character[i][4].push('db:extension-群雄乱舞:'+i+'.jpg');}
			}
			return xu;
		});
...
connect:true,		
                ...
                image:"ext:群雄乱舞/xwj_xus_houzi.png",
                enable:true,
                selectTarget:-1,
                filterTarget:function (card,player,target){
        return target==player;
...
image:"ext:群雄乱舞/zbfs.png",				
        ...
        target.addJudge(card,cards);
            ...
            return 0;
                ...
                image:"ext:群雄乱舞/xwj_xus_mianju.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,               
...
image:"ext:群雄乱舞/xwj_xus_shoulijian.png",				
                ai:{
                    basic:{
                        order:9,
                        value:6,
            ...
            "xwj_xus_kuwu":{
                audio:true,
                fullskin:true,
				image:"ext:群雄乱舞/xwj_xus_kuwu.png",
                type:"equip",
                subtype:"equip1",
                skills:["xwj_xus_kuwu"],
                nomod:true,
            ...
            "xwj_xus_xuelunyang":{
                ...
                image:"ext:群雄乱舞/xwj_xus_xuelunyang.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,                
            ...
            "xwj_xus_jiuwei":{
                ...
                image:"ext:群雄乱舞/xwj_xus_jiuwei.png",
                enable:true,
                selectTarget:-1,                
                modTarget:true,
                allowMultiple:false,               
...
if(!lib.config.cards.contains('xwj_xus_equip')) lib.config.cards.remove('xwj_xus_equip');
    ...
    intro:"若想关闭某个扩展小包，可在相应武将栏内关闭并重启，开启同理。更多介绍请看：其它→帮助",
    author:"★Sukincen★",
    diskURL:"",
    forumURL:"",
    version:"2.51",
